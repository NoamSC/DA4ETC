...
**Citation:** Danil Shamsimukhametov, Anton Kurapov, Mikhail Liubogoshchev, Evgeny Khorov — *IEEE Access* 12, **2024** (DOI 10.1109/ACCESS.2024.3469730). `shamsimukhametov2024ech`
...
## 9. Website / mobile-app fingerprinting & per-class drift (Supporting / Background — added by m12 citation audit)

*These four entries were added during the m12 citation audit to vet in-text characterizations in `paper/main.tex` that previously had no SUMMARIES entry. HOLMES was verified against the extracted full text of the CCS'24 PDF (arXiv:2407.00918); the other three against the published abstract/HTML and authors' code repos. All four in-text characterizations were found ACCURATE.*

### FG-Net: Accurate Mobile-App Fingerprinting via Flow-level Relationships with GNNs (jiang2022fgnet)
**Citation:** Minghao Jiang, Zhen Li, Peipei Fu, Wei Cai, Mingxin Cui, Gang Xiong, Gaopeng Gou — *Computer Networks* 217, art. 109309, 2022. `jiang2022fgnet`
**TL;DR:** A mobile-app fingerprinting attack that builds a flow-relationship graph (FRG) embedding raw packet-level info plus inter-flow structure, classified with a GNN, because flow-level relationships are distinctive across apps yet **stable across app versions**.
**What it does:** Targets two real-world failures of packet-level fingerprinting — common/shared traffic across apps, and traffic drift from app updates. FRG + GNN gives a representation that survives version churn; reports the most robustness to updated traffic **without retraining**, +5.5% accuracy over SOTA, and gains on ~18% common traffic.
**Relevance (Supporting):** Independent encrypted-traffic evidence that per-application version updates drive per-class signature shifts (cited at main.tex ~L443 for exactly this point). Its response is robustness-oriented — a *drift-invariant representation* — not a label-free monitor that localizes which class broke, so it sits adjacent to, not overlapping with, our niche. Characterization in main.tex (~L360-361, "seeking drift-invariant representations") is **ACCURATE**.

### NetCLR: Realistic Website Fingerprinting by Augmenting Network Traces (bahramali2023netclr)
**Citation:** Alireza Bahramali, Ardavan Bozorgi, Amir Houmansadr (UMass Amherst) — *ACM CCS 2023*, pp. 1035–1049 (arXiv:2309.10147; DOI 10.1145/3576915.3616639). `bahramali2023netclr`
**What it does:** Introduces NetAugment, an augmentation tailored to Tor traces (simulating bandwidth/latency/circuit variation), instantiated via self-/semi-supervised contrastive pre-training (NetCLR) so a WF classifier generalizes to network conditions unobserved at training time. 5-shot closed-world: up to ~80% accuracy on unobserved-setting traces vs ~64.4% for Triplet Fingerprinting.
**Relevance (Supporting / Background):** A *realism- and augmentation-robust training* response to deployment distribution shift — make the classifier resilient, rather than monitor and localize degradation. Cited at main.tex ~L361-362 (and ~L255 with TESSERACT on evaluation realism). Characterization "augmentation- and realism-robust training" is **ACCURATE**.

### ARES: Robust Multi-tab Website Fingerprinting Attacks in the Wild (deng2023ares)
**Citation:** Xinhao Deng, Qilei Yin, Zhuotao Liu, Xiyuan Zhao, Qi Li, Mingwei Xu, Ke Xu, Jianping Wu — *IEEE S&P 2023*, pp. 1005–1022. `deng2023ares`
**What it does:** Formulates multi-tab WF as **multi-label classification** solved by a **multi-classifier (one-vs-all) framework**: N parallel Transformer-based Trans-WF models, one per monitored website, each deciding whether *its* website is present in the mixed traffic. Uses local patterns from multiple traffic segments; robust against several WF defenses.
**Relevance (Background):** The per-website / per-classifier decomposition is what main.tex (~L362-363, ~L1215) invokes as a precedent for *targeted per-classifier updates rather than holistic retraining* — adding/refreshing one website's classifier need not touch the others. Characterization is **ACCURATE** (the per-website classifier design is explicit; "targeted update vs holistic retrain" is a reasonable operational reading of that design rather than a verbatim claim).

### HOLMES: Robust & Reliable Early-Stage Website Fingerprinting via Spatial-Temporal Distribution Analysis (deng2024holmes)
**Citation:** Xinhao Deng, Qi Li, Ke Xu — *ACM SIGSAC CCS 2024*, Salt Lake City, UT. `deng2024holmes`
**What it does:** An early-stage WF **attack** (identifies a site from partial page loads, avg ~21.71% loaded). **Supervised** contrastive learning maps traffic into an embedding space; **spatial distribution analysis** via Median Absolute Deviation defines, per monitored website, a centroid + radius decision region; low-confidence / out-of-region traffic is **rejected**. Temporal-distribution adaptive data augmentation handles incompleteness.
**Verified specifics (all three main.tex claims hold):** (i) **Supervised — labels define the regions:** yes, supervised contrastive learning + per-website spatial regions. (ii) **Spatial decision regions with out-of-region rejection:** yes ("Holmes rejects... low-confidence identifications"; centroid/radius regions via MAD). (iii) **Per-class region UPDATES on drift:** yes, and explicitly drift-triggered, not merely early-stage — §"Concept Drift": "We only collect traffic from websites with concept drift and update the centroid and radius of the corresponding websites," with a per-website threshold ε on (distance − radius) detecting drift samples per website. HOLMES is supervised and an attack; ours is label-free, frozen-model, and quantifies the harm of global adaptation.
**Relevance (Background — closest cousin to our framing):** Already instantiates the per-class open-set view (spatial regions, rejection, per-class drift updates) in encrypted traffic; we borrow the lens, not the algorithm. main.tex's detailed characterization (~L402-408) is **ACCURATE** in every flagged specific.
