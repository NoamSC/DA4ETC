...
### DANN forward-transfer (diagonal) — week-16 source

_Each week scored by its own source-16→week-N DANN-aligned model (transductive, one model
per week — the most favorable setting for DANN); forward weeks 16–52._
...
frozen source is negligible (+0.006 mean, +0.001 on far weeks) — global domain alignment does
not recover the per-class discrete drift.
...
5. **DANN does not transfer forward even in its best case.** Scored on the diagonal (each week
   by its own dedicated source-16→week-N aligned model — transductive, one model per week),
   DANN beats the frozen source by only **+0.006** mean and **+0.001** on far weeks (≥43).
   Global source–target alignment cannot recover the per-class *discrete* drift the paper
   characterizes; the apparent in-distribution health does not generalize across time.
