...
def safe_parse_ppi(x):
    ...
    try:
        ...
        if isinstance(ppi, list) and len(ppi) >= 3:
            ...
            directions = ppi[1] if isinstance(ppi[1], list) else []
            sizes = ppi[2] if isinstance(ppi[2], list) else []
            return ipt, directions, sizes
...
def process_csv_file(csv_path, overwrite=False):
    ...
    try:
        ...
        df['PPI_DIRECTIONS'] = parsed.apply(lambda x: x[1])
        df['PPI_SIZES'] = parsed.apply(lambda x: x[2])
