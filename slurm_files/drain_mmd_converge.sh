#!/bin/bash
# Drive the MMD sweep to convergence (50 epochs) despite killable
# preemption. Resubmits the array (resume from rolling checkpoints) whenever no
# mmd_fwd_w16 job is queued and not all 52 targets have reached 50 epochs.
...
import glob
n=tot=0
for d in glob.glob("exps/cesnet_tls_mmd_fwd_w16_v01/*/tensorboard"):
    tot+=1
...
PYEOF
  ...
  inq=$(squeue -u "$USER" -h -o "%j" | grep -c mmd_fwd_w16)
  echo "[try $i] converged(>=50ep): $done ; dann in queue: $inq"
  if [ "${ndone:-0}" -ge "${ntot:-52}" ] && [ "${ntot:-0}" -ge 52 ]; then echo "ALL MMD TARGETS CONVERGED"; exit 0; fi
  ...
  if [ "$inq" -eq 0 ]; then
    jid=$(sbatch --parsable slurm_files/run_mmd_fwd_w16.slurm 2>/dev/null) && echo "  resubmitted MMD array -> $jid" || echo "  resubmit deferred (QOS cap)"
  fi
  sleep 1800
done
