...
class GeM(nn.Module):
    ...
    def gem(self, x, p: Union[int, nn.Parameter] = 3, eps=1e-6):
...
class Multimodal_CESNET(nn.Module):
    def __init__(self, num_classes: int,
                       ...
                       lambda_rgl: float = 0.0, lambda_grl_gamma: float = 10.0,
                       domain_classifier_hidden: int = 256, domain_classifier_dropout: float = 0.5,
                       ):
        super().__init__()
        ...
        # DANN components
        self.lambda_rgl = lambda_rgl
        self.lambda_grl_gamma = lambda_grl_gamma
        self.epoch = 0

        if lambda_rgl > 0:
            self.grl = GradientReversalLayer(lambda_=0.0)
            self.domain_classifier = nn.Sequential(
                nn.Linear(mlp_shared_size, domain_classifier_hidden),
                nn.ReLU(inplace=False),
    ...
    def set_epoch(self, epoch: int):
        """Set current epoch. Used for scheduling GRL lambda."""
        self.epoch = epoch
        if self.lambda_rgl > 0 and self.grl is not None:
            self.grl.lambda_ = compute_grl_lambda(epoch, self.lambda_rgl, self.lambda_grl_gamma)
    ...
    def get_grad_norms(self) -> dict:
        ...
        # Feature extractor includes CNN and MLP components
        feature_params = list(self.cnn_ppi.parameters()) + list(self.mlp_flowstats.parameters()) + list(self.mlp_shared.parameters())
        if self.cnn_ppi_use_pooling:
            feature_params += list(self.cnn_global_pooling.parameters())
        else:
            feature_params += list(self.cnn_flatten_without_pooling.parameters())

        result = {
            'feature_extractor': compute_norm(feature_params),
            'class_predictor': compute_norm(self.classifier.parameters()),
        }
        if self.domain_classifier is not None:
            result['domain_classifier'] = compute_norm(self.domain_classifier.parameters())
        return result
    ...
    def forward(self, *x: tuple) -> dict:
        if len(x) == 1:
            x = x[0]
        ppi, flowstats = x

        # ppi has to be torch.Size([B, 3, 30]), flowstats has to be torch.Size([8, 44])
        assert ppi.shape[1] == self.ppi_input_channels and ppi.shape[2] == 30, f"PPI input shape mismatch: expected channels={self.ppi_input_channels}, length=30 but got {ppi.shape}"
        assert flowstats.shape[1] == self.flowstats_input_size, f"Flowstats input shape mismatch: expected size={self.flowstats_input_size} but got {flowstats.shape}"

        features = self.forward_features(ppi=ppi, flowstats=flowstats)
        class_preds = self.forward_head(features)

        # Domain prediction with gradient reversal
        if self.grl is not None and self.domain_classifier is not None:
